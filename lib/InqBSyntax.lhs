...
relation \verb|myUnR| from Section \ref{sec:Models}. This formula corresponds
...
formula look like in Haskell, we can create arbitrary instances of them.
...
The arbitrary model will contain an arbitrary subset of these. As the \verb|Arbitrary| instance
...
the type \verb|UnRelation|, called \verb|ur|. We have chosen to only include one relation of each 
arity in our arbitrary models, but this could be extended to an arbitrary number.
The code for binary, \verb|ur|, and tertiary, \verb|tr|, relations is 
analogous.\footnote{Note that these sets are not shuffled. This is because the shuffling of 
these larger sets made the code very slow. A possible improvement would be a 
...
that will be quantified over. However, given our current purposes and time constraints 
