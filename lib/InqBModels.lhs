...
This allows us to implement a universe and a domain in a principled manner.
\begin{code}
...
However, in natural language we rarely encounter relations of an arity higher than three.
We have therefore chosen to only implement unary, binary and tertiary relations. For example,
the unary relation is represented as the characteristic set of a function from worlds to sets of 
individuals.\footnote{Note that we represent sets as lists in Haskell.}
...
mathematically less complete, it allows for an intuitive way of defining models.
